...
$('#thumbnail-delete').click(function () {
	if(act_del===false) {
		act_del= true;
	} else {
		act_del= false;
...
$(document).on('click', '.file-item', function (e) {
	if(act_del===true) {
		var file_name= get_file_name($(this).data('id'));
		performLfmRequest('delete', {items: file_name}).done(refreshFoldersAndItems);
	} else {
...
function createFolder(folder_name) {
  performLfmRequest('newfolder', {name: slug(folder_name)})
